...
**Open implementation detail** (to resolve during planning, not blocking this design): how a cross-skill credit is recorded in `Assignment`/`Participant` — e.g. whether a credited volunteer gets an `Assignment` row against the other `Need` too, for later collective-confirmation/quorum purposes, or is tracked separately. Needs a concrete answer in the implementation plan.
